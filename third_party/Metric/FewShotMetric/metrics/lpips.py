...
#----------------------------------------------------------------------------
def compute_clpips_between_dir(opts, num_gen):
    dataset_kwargs  = opts.dataset_kwargs
    dataset2_kwargs = opts.dataset2_kwargs
    device = opts.device
    ...
    with torch.no_grad():
        ...
        cluster  = [[] for _ in range(data_list.shape[0])]
        dataset2 = dnnlib.util.construct_class_by_name(**dataset2_kwargs)
        for images, _labels in torch.utils.data.DataLoader(dataset=dataset2, batch_size=1,**data_loader_kwargs):
            score_list    = loss_fn_alex(images.repeat(data_list.shape[0], 1, 1, 1).to(device), data_list)
            closest_index = score_list.argmin().item()
            if len(cluster[closest_index]) < 200:
                cluster[closest_index].append(img)
            del score_list
        ...
        for c in iterator:
            ...
            for _ in range(img.shape[0] - 1):
                img = torch.cat([img[1:], img[0:1]], dim = 0)
                c_lpips += loss_fn_alex(img, ref_img).sum().item()
                print("_ = {}, range = {}".format(_,img.shape[0] - 1))
            cluster_lpips.append(c_lpips / (img.shape[0] * (img.shape[0] - 1)))
            i+=1
            print("{} in {}".format(i,num))
            del img
            del c
