...
#----------------------------------------------------------------------------
@register_metric
def fid5k_between_dir(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    fid = frechet_inception_distance.compute_fid_between_dir(opts, max_real=None, num_gen=5000)
    return dict(fid_between_dir=fid)

@register_metric
def kid5k_between_dir(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
...
@register_metric
def clpips1k_between_dir(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    clpips1k, clpips1k_rz = lpips.compute_clpips_between_dir(opts, num_gen = 1000)
    return dict(clpips1k = clpips1k, clpips1k_rz = clpips1k_rz)


@register_metric
def is5k_between_dir(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    mean, std = inception_score.compute_is_between_dir(opts, num_gen=5000, num_splits=10)
    return dict(is5k_mean=mean, is5k_std=std)

@register_metric
def pr50k3_between_dir(opts):
    opts.dataset_kwargs.update(max_size=None)
    precision, recall = precision_recall.compute_pr_between_dir(opts, max_real=50000, num_gen=50000, nhood_size=3, row_batch_size=10000, col_batch_size=10000)
